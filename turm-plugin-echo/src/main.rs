...
//! (`TURM_ECHO_HEARTBEAT_SECS`, defaulting to 30s — short during E2E,
...
fn main() {
    ...
    // out before turm has finished the handshake.
    let initialized = Arc::new(AtomicBool::new(false));
    let interval_secs: u64 = std::env::var("TURM_ECHO_HEARTBEAT_SECS")
        .ok()
...
fn handle_frame(value: &Value, tx: &Sender<String>, initialized: &AtomicBool) {
    ...
    match method {
            ...
            match name {
                "echo.ping" => {
                    send_response(
                        tx,
                        id,
                        json!({ "echoed": action_params, "from": "turm-plugin-echo" }),
                    );
                }
                other => {
                    send_error(
        ...
        "event.dispatch" => {
            ...
            // protocol allows turm to forward events for future
