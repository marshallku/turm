...
impl SearchBar {
    pub fn new(terminal: &vte4::Terminal) -> Self {
        ...
        case_btn.add_css_class("custerm-search-btn");
        case_btn.set_tooltip_text(Some("Case sensitive"));

        let close_btn = gtk4::Button::from_icon_name("window-close-symbolic");
        close_btn.add_css_class("flat");
        close_btn.add_css_class("custerm-search-btn");
...
.custerm-search-entry:focus {
    border-color: #89b4fa;
}

.custerm-search-count {
    color: #6c7086;
    font-size: 12px;
    margin: 0 4px;
}
