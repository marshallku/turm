//! First-party Slack service plugin for turm.
//!
...
//! `turm-plugin-slack auth` while turm is running and the loop
...
#[cfg(not(unix))]
compile_error!(
    "turm-plugin-slack is currently Unix-only. The keyring crate's mock fallback \
...
fn main() {
    ...
    match args.next().as_deref() {
        ...
        Some(other) => {
            ...
            eprintln!("usage: turm-plugin-slack [auth]");
            std::process::exit(2);
        }
        None => run_rpc(),
    }
...
fn run_auth() {
    ...
    if config.bot_token.is_empty() {
        eprintln!("[slack] auth requires TURM_SLACK_BOT_TOKEN (xoxb-...)");
        std::process::exit(1);
    }
    if config.app_token.is_empty() {
        eprintln!("[slack] auth requires TURM_SLACK_APP_TOKEN (xapp-...)");
        std::process::exit(1);
    }
...
fn run_rpc() {
    ...
    // running `turm-plugin-slack auth` while turm is already up
