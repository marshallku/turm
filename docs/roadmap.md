...
- [x] Background random rotation (`BackgroundRotator` reads `~/Library/Caches/turm/wallpapers.txt`, fallback `~/.cache/terminal-wallpapers.txt`; mode file `~/Library/Caches/turm/bg-mode`; sockets `background.next` / `background.toggle` — see [macos-parity-plan.md Tier 1.3](./macos-parity-plan.md))
...
- [x] **`turm-plugin-todo` Rust workspace member** (Linux + macOS via `any(linux, macos)` gate, same posture as `turm-plugin-kb`; atomic-create rename routed through `turm_core::fs_atomic::rename_no_replace`, `O_NOFOLLOW` works unchanged on both). Activation `onStartup` so the file-watcher is alive whenever turm runs (`onAction:todo.*` would only catch programmatic edits, missing vim writes).
...
- [x] **`turmctl todo`** subcommands (slice 19.1a, shipped): `create <title> [--body] [--priority] [--workspace] [--due] [--linked-jira] [--tags]`, `list [--status] [--workspace] [--tag] [--due-before] [--hide-done]`, `set <id> --status <open|in_progress|blocked|done> [--workspace <ws>]`, `start <id> [--workspace]`, `delete <id> [--workspace]`. Shorthands `done <id>` / `doing <id>` / `block <id>` desugar to `set --status …`. ID-prefix matching ships: bare prefix matches uniquely (`turmctl todo done T-2026` matches `T-2026...0042` if unambiguous), and the preflight `todo.list` call resolves the workspace alongside the id so a todo in a non-default workspace works without `--workspace`. Cross-workspace duplicates: ids are workspace-scoped (the store at `<root>/<workspace>/<id>.md` only checks collisions per workspace), so the CLI collects ALL exact matches and forces disambiguation via `--workspace <ws>` (or a longer prefix) rather than silently picking whichever workspace `todo.list` enumerated first. Workspace default for `create` falls through `--workspace` flag → `TURM_TODO_DEFAULT_WORKSPACE` env → plugin's own default. Cwd-derived workspace resolution stays as a 19.X follow-up (needs a turm-internal `resolve_workspace(cwd)` action). Implementation: `turm-cli/src/plugin_cmds/todo.rs` (~360 LOC); dispatched from `main.rs` via `Command::Todo` shortcut so the generic `cli.method() / cli.params()` path stays untouched.
- [~] ~~**`turmctl kb`** subcommands: `new <id> [--title] [--from-stdin]`, `cat <id>` (renders the markdown), `search <query> [--limit]`, `append <id> <text>` (calls `kb.append` with `ensure=true`), `list <prefix>`. Composes the existing kb actions; no new plugin work.~~ **Dropped.** Every operation is redundant with filesystem tools (`vim ~/docs/kb/<id>.md`, `cat`, `rg`, `ls ~/docs/kb/`); the kb plugin's actual value lives in OTHER plugins driving `kb.append` automatically (slack/discord 📝-reaction capture, calendar pre-meeting briefs, todo `linked_kb`) and in claude/codex agents reading `~/docs/` directly via filesystem. Scripts that genuinely need to drive kb actions can use the existing `turmctl call kb.<name> --params '{...}'` escape hatch. A turmctl-kb wrapper would be CLI surface area without ergonomic gain.
...
- [x] **`turmctl git`** subcommands (slice 19.1b, shipped): `workspaces`, `worktrees [--workspace]`, `wt add <branch> [--workspace] [--sanitize-jira]`, `wt remove <path> [--force]`, `branch [--workspace]`, `status [--workspace | --path]`. Wraps `git.list_workspaces` / `git.list_worktrees` / `git.worktree_add` / `git.worktree_remove` / `git.current_branch` / `git.status`. **Cwd-derived workspace defaulting** ships here ahead of the 19.X follow-up — every command that takes `--workspace` falls through `flag → TURM_GIT_DEFAULT_WORKSPACE env → cwd-prefix-match against either workspace.path OR workspace.worktree_root (longest-prefix wins, so a worktree under <repo>-worktrees resolves correctly) → single-config-entry → error with candidate list (since the plugin's require_workspace surface returns not_found without enumerating candidates, the CLI does the listing client-side)`. Status renderer shows `<branch> → <upstream> <ahead>↑<behind>↓ clean/dirty` plus staged/unstaged/untracked counts. Worktree renderer shows `<head8> <branch> <path>` with `[locked]` / `[prunable]` tags. Implementation: `turm-cli/src/plugin_cmds/git.rs` (~340 LOC).
...
- [x] **`turmctl todo show <id>`** (slice 19.2b, shipped): full Todo payload + linked-entity expansion. Composes existing actions only — `todo.list` (workspace-filtered, then id-pick after the existing prefix resolver) + `kb.read` per `linked_kb` entry (best-effort, per-entry errors swallowed and surfaced inline). Human render: title/status/priority/tags/body/prompt + a 5-line preview per kb note (frontmatter stripped, blank-leading lines skipped). `linked_jira` shows the key verbatim; the planned `jira.get_ticket` fan-out is gated on Phase 16. `linked_slack` permalinks render as-is. Timeline (todo.changed / todo.completed / todo.start_requested) deferred to slice 19.2c — needs `event.history` ring buffer, no socket-callable history surface today. `--json` returns the aggregate (todo + linked_kb_resolved). Implementation: extends `turm-cli/src/plugin_cmds/todo.rs` (~225 LOC added).
- [x] **`turmctl context` expansion** (slice 19.2a, shipped): default `turmctl context` (human mode) and `turmctl context --full` aggregate `context.snapshot` + `session.info` + `git.list_workspaces` (cwd-derived workspace) + `git.status` + `todo.list` (open + in-progress, capped 10) + `calendar.list_events --lookahead 2` + `slack.auth_status` + `discord.auth_status` into one dense view. `turmctl --json context` (no `--full`) keeps the raw `{active_panel, active_cwd}` shape for backward compatibility with scripts already piping it; `--json --full` emits the aggregate as one object. Each section degrades to `(unavailable)` on its own — a lazy-spawn timeout in calendar doesn't break the rest. Workspace resolution mirrors `turmctl git`'s cwd-derive (longest-prefix vs `path` OR `worktree_root`, canonicalized); cwd outside any workspace cleanly skips the workspace-bound sections rather than guessing. `slack.list_unseen_mentions` deferred (needs a new slack action — slips to 19.3). Implementation: `turm-cli/src/plugin_cmds/context.rs` (~290 LOC).
...
- [x] **`turm-plugin-bookmark` workspace member**. Crate skeleton (`Cargo.toml` deps: `serde`, `serde_json`, `chrono`, `sha1`, `url`, `libc`, `turm-core` for `fs_atomic`), stdio JSON dispatcher (mirrors `turm-plugin-kb`), Linux + macOS via `any(linux, macos)` compile guard (atomic-create rename routes through `turm_core::fs_atomic::rename_no_replace`).
...
- [x] **`discord.auth_status` action**. Returns `{configured, authenticated, credentials_source, store_kind, workspace, user_id, username, fatal_error}`. Mirrors Slack's `slack.auth_status` so a future `turmctl context --full` can surface both workspaces uniformly. (Slice 2 added `credentials_source` + `fatal_error` for symmetry with Slack's reporting.)
- [x] **Plugin manifest** at `examples/plugins/discord/plugin.toml`. `onStartup` activation (Gateway WebSocket lives whenever turm runs). Slice 1 shipped `provides = ["discord.auth_status"]`; slice 2 added `discord.send_message`. Required scopes / OAuth flow / MESSAGE CONTENT intent setup documented inline in the manifest.
