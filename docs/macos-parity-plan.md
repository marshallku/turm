...
7. **PR 5c: TriggerEngine via FFI** — wire `event.publish` from plugin → EventBus → trigger match → action dispatch. Demo target: echo's `system.heartbeat` (every 30s) → user-defined trigger → `system.ffi_test` action. Once that works, calendar's `event_imminent` events feed the same chain (notify on meeting, etc.). Real lazy `onAction:<glob>` activation lands here too (currently eager-spawned with a log).
