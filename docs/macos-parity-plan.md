...
- **3.3 Plugin platform gates** — KB/Todo/Bookmark blocked by codex finding (Linux-only `renameat2` / `O_NOFOLLOW`); other plugins compile on macOS but auth/Keychain UX unverified. PR 4 picks one (`git` or `llm.auth_status`) to validate beyond echo.
...
- **3.5 Plugin install on macOS** ✅ (PR 3, scoped) — folded into `install-macos.sh` rather than a separate `install-plugins-macos.sh`. New `--no-plugins` flag opts out. Currently only echo is in the macOS-buildable list (`MACOS_PLUGINS=(echo)`); kb/todo/bookmark won't compile, slack/calendar/llm/discord need PR 4 verification before opt-in. Code-signing punted — locally-built binaries don't carry quarantine xattr so Gatekeeper doesn't block them.
