# Service Plugins: nestty as a Plugin-First Personal Workflow Runtime
...
At maturity, nestty is a **plugin host** for personal workflow automation. The binaries shipped (`nestty`, `nestctl`) own the runtime primitives only — Event Bus, Action Registry, Trigger Engine, Context Service, Plugin Loader. Every domain integration (KB, Calendar, Slack, Discord, Notion, LLM, …) lives outside `nestty-core`/`nestty-linux` as a service plugin: an independent process with its own release cadence, language choice, dependencies, and credentials. The user can swap KB backends (file-based vs Notion vs Obsidian-vault), pick which messenger they want ingested, and write their own integrations without rebuilding nestty.
...
4. The user types `nestctl draft-reply <topic>`. CLI invokes a registry action that asks the LLM plugin (with KB context as input) to produce a draft reply, opening it in the active terminal.

None of this requires changes to `nestty-core`. New integrations are new plugins.
...
Without service plugins, every integration would need to live inside `nestty-core` or `nestty-linux`. Calendar OAuth flow, Slack WebSocket, Notion API, LLM credential management — all in core. That makes nestty a kitchen-sink monolith, locks the user to whatever backends nestty chooses (no Obsidian-vault KB, no per-user LLM provider), and makes contribution from outside the main repo very hard.
...
- WASM (Zed) is the alternative but adds significant complexity (Wasmtime runtime, WIT compilation step) that personal-scale nestty doesn't yet need

cmux V2 newline-delimited JSON is already proven in nestty. Sticking with it avoids two protocols. LSP-style `Content-Length` header framing is unnecessary as long as JSON serializers don't emit raw newlines (serde_json doesn't).
...
```toml
# ~/.config/nestty/plugins/kb/plugin.toml
...
exec = "nestty-plugin-kb"            # binary in PATH or relative to plugin dir
...
```
nestty spawns the service binary.

nestty → service (stdin):
{
  "id": "init-1",
  "method": "initialize",
  "params": {
    "nestty_version": "0.x.y",
    "protocol_version": 1
  }
}

service → nestty (stdout):
{
  "id": "init-1",
  "ok": true,
  "result": {
...
If `provides` conflicts with another loaded plugin, nestty logs a warning and skips registration of the conflicting names; non-conflicting names still register.
...
- Wire bidirectional RPC: nestty→service `action.invoke` / `event.dispatch`; service→nestty `event.publish` / `log`
...
- Credentials: `ANTHROPIC_API_KEY` env or keyring (Linux Secret
  ...
  by `NESTTY_LLM_REQUIRE_SECURE_STORE`. NOT the abandoned
  `~/.config/nestty/secrets.toml` design — every credential-bearing
  plugin has converged on the keyring-or-plaintext pattern.
- `llm.usage` aggregates token counts from a JSONL log at
  `$XDG_DATA_HOME/nestty/llm-usage-<account>.jsonl`. No USD cost
...
2. Phase 17 (Git **worktree** plugin) — single-action `git.worktree_add` already useful via `nestctl call`
...
- **Plugin distribution.** First-party plugins ship in the nestty git repo as separate Cargo crates? Or fully external repos? Initially: same repo, separate crates. Distribution mechanism (registry, install command) is post-MVP.
...
- ~~**Authentication / per-user secrets.** Calendar OAuth, Slack tokens. Where do they live? Probably `~/.config/nestty/secrets.toml`...~~ **Resolved (Phase 10–12)**: every credential-bearing plugin uses the `keyring` crate (Linux Secret Service via D-Bus, macOS Keychain) with plaintext 0600 fallback at `$XDG_CONFIG_HOME/nestty/<plugin>-token-<account>.json`. Account label scoped via env var (`NESTTY_<PLUGIN>_ACCOUNT`). `<PLUGIN>_REQUIRE_SECURE_STORE=1` opt-in refuses plaintext fallback. The `~/.config/nestty/secrets.toml` shared-file design was abandoned — per-plugin keyring entries are simpler and prevent one plugin's credentials from leaking through another's process boundary.
- **Multi-instance nestty.** Currently socket per PID. Each nestty spawns its own copies of all service plugins? Or shared daemons? For v1, per-instance. Revisit if plugin spawn cost matters.
...
- nestty-core module reference: [core-lib.md](./core-lib.md)
