...
- **End-to-end Phase 14 chain test (Vision Flow 3)**: `todo.start_requested` → `git.worktree_add {prompt: "{event.assembled_prompt}"}` → `git.worktree_add.completed` (carries the forwarded prompt via the new passthrough field as `event.prompt` on the completion event) → `claude.start {workspace_path, prompt: "{event.prompt}"}` works today end-to-end on the FIRST click. Tab pops, claude opens in a fresh tmux session inside the new worktree, and the layered prompt is pre-pasted + submitted. **Re-clicking Start on the same Todo currently fails at `git.worktree_add` with `branch_exists`** — making `git.worktree_add` idempotent on existing worktrees is a small follow-up that would close the "resume work" UX. Jira summary fan-in still pending Phase 16 + 14.2.
