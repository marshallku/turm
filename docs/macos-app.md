...
`SocketServer.swift`는 POSIX `socket`/`bind`/`listen`/`accept` + 전용 `Thread`로 구현됩니다. 각 클라이언트마다 별도 Thread를 생성하고, 커맨드 처리는 `DispatchQueue.main.sync`로 메인 스레드에 **동기** 디스패치합니다. Linux의 `glib::timeout_add_local` 50ms 폴링 방식과 달리 직접 동기 호출이므로 응답 지연이 없습니다.

프로토콜은 turm-core `protocol.rs`와 동일: newline-delimited JSON (`Request` → `Response`).

**지원 커맨드:** `system.ping`, `terminal.exec`, `terminal.feed`, `terminal.read`, `terminal.state`
