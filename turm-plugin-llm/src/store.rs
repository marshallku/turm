...
//! at `$XDG_CONFIG_HOME/turm/llm-token-<account>.json`. The
...
const KEYRING_SERVICE: &str = "turm-llm";
    ...
    match KeyringStore::open(&config.account_label) {
        ...
        Err(e) => {
            if config.require_secure_store {
                eprintln!(
                    "[llm] secure keyring unavailable AND TURM_LLM_REQUIRE_SECURE_STORE=1: {e}"
                );
