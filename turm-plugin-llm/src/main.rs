//! First-party LLM service plugin for turm (Anthropic provider).
//!
//! Two run modes:
...
//! spawns when a trigger or `turmctl call` invokes one of the
//! actions. Cold-start cost is dominated by the first HTTP call
//! anyway.

#[cfg(not(unix))]
compile_error!(
    "turm-plugin-llm is currently Unix-only. The keyring crate's mock fallback \
...
fn main() {
    ...
    match args.next().as_deref() {
        ...
        Some(other) => {
            ...
            eprintln!("usage: turm-plugin-llm [auth]");
            std::process::exit(2);
        }
        None => run_rpc(),
    }
    ...
    if let Err(e) = anthropic::validate_key(&config.api_key, &config.default_model) {
        eprintln!(
            "[llm] validation failed: {e}\n\
             [llm] confirm ANTHROPIC_API_KEY is correct and TURM_LLM_DEFAULT_MODEL \
             is a valid model id"
        );
        std::process::exit(1);
    }
...
fn handle_action(
    ...
    match name {
        ...
        // a bad TURM_LLM_ACCOUNT silently falls back to the
        ...
        "llm.usage" => {
            if !config.account_resolved {
                return Err((
                    ...
                    format!(
                        "TURM_LLM_ACCOUNT is invalid; refusing to read llm-usage-default.jsonl as a stand-in. Fix: {}",
...
mod tests {
    ...
    fn usage_refuses_when_account_unresolved() {
        ...
        c.fatal_error = Some("TURM_LLM_ACCOUNT: invalid character ...".into());
