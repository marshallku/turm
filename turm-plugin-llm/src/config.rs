...
/// param or globally via TURM_LLM_DEFAULT_MODEL.
const DEFAULT_MODEL: &str = "claude-sonnet-4-6";

/// Maximum allowed `TURM_LLM_HTTP_TIMEOUT_SECS`. Must leave margin
/// under the supervisor's `DEFAULT_ACTION_TIMEOUT` (120s in
/// turm-linux/src/service_supervisor.rs) so a successful plugin
...
impl Config {
    ...
    /// `TURM_LLM_ACCOUNT` doesn't silently redirect `llm.usage` to
    ...
    pub fn from_env() -> Self {
        ...
        // Reject `TURM_LLM_DEFAULT_MODEL=""` — using the empty
        ...
        let default_max_tokens =
            parse_nonzero_int_with_default("TURM_LLM_DEFAULT_MAX_TOKENS", 4096u32, &mut errors);
        let mut http_timeout_secs: u64 = parse_nonzero_int_with_default(
            "TURM_LLM_HTTP_TIMEOUT_SECS",
            MAX_HTTP_TIMEOUT_SECS,
            &mut errors,
        );
        if http_timeout_secs > MAX_HTTP_TIMEOUT_SECS {
            errors.push(format!(
                "TURM_LLM_HTTP_TIMEOUT_SECS={http_timeout_secs} exceeds supervisor cap \
                 ...
                 turm-linux/src/service_supervisor.rs first if you genuinely need longer."
            ));
            http_timeout_secs = MAX_HTTP_TIMEOUT_SECS;
        }
        let require_secure_store =
            parse_bool_with_default("TURM_LLM_REQUIRE_SECURE_STORE", false, &mut errors);
...
pub fn validate_account_label(s: &str) -> Result<(), String> {
    if s.is_empty() {
        return Err("TURM_LLM_ACCOUNT: cannot be empty".to_string());
    }
    if s == "." || s == ".." {
        return Err(format!(
            "TURM_LLM_ACCOUNT: {s:?} is reserved (use a normal label)"
        ));
    }
    for c in s.chars() {
        ...
        if !ok {
            return Err(format!(
                "TURM_LLM_ACCOUNT: invalid character {c:?} \
