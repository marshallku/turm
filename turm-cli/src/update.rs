...
const REPO: &str = "marshallku/turm";
...
fn client() -> reqwest::blocking::Client {
    reqwest::blocking::Client::builder()
        .user_agent(format!("turm/{CURRENT_VERSION}"))
        .build()
...
pub fn check_update() {
    ...
    if latest > current {
        ...
        println!("Run `turmctl update apply` to install.");
    } else {
...
pub fn apply_update(version: Option<String>) {
    ...
    let asset_name = format!("turm-{}-x86_64-linux.tar.gz", release.tag_name);
    let asset = release
        .assets
        .iter()
    ...
    println!("Downloading turm v{target_version}...");
    ...
    println!("turm v{target_version} installed successfully!");
}
