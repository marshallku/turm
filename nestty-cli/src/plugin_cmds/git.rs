//! `nestctl git` — ergonomic wrapper over the `git.*` action surface.
//!
...
//! 2. `NESTTY_GIT_DEFAULT_WORKSPACE` env var.
...
//! `nestctl git status`, get the right answer without typing
//! `--workspace`. Implemented client-side via the preflight call;
//! a future Phase 19.X nestty-internal `resolve_workspace(cwd)` action
...
fn resolve_workspace(socket_path: &str, explicit: Option<&str>) -> Result<String, i32> {
    ...
    if let Ok(env_ws) = std::env::var("NESTTY_GIT_DEFAULT_WORKSPACE")
        && !env_ws.is_empty()
    {
        return Ok(env_ws);
    }
    ...
    if arr.is_empty() {
        ...
        if let Some(err) = fatal {
            ...
            eprintln!("Error: no workspaces configured. Edit ~/.config/nestty/workspaces.toml.");
        }
        return Err(1);
    }
    // Canonicalize cwd once so symlinked entry into a workspace
    // (e.g. user has `~/work/nestty -> /home/marshall/dev/nestty`) still
    ...
    eprintln!(
        "       Pass `--workspace <name>`, set NESTTY_GIT_DEFAULT_WORKSPACE, or `cd` into one of:"
    );
    for w in &arr {
