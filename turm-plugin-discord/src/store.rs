...
//! `$XDG_CONFIG_HOME/turm/discord-token-<workspace>.json` (mode 0600,
//! atomic-replace via per-call temp + rename) with a stderr warning
//! on every open. `TURM_DISCORD_REQUIRE_SECURE_STORE=1` forbids the
...
const KEYRING_SERVICE: &str = "turm-discord";
    ...
    match KeyringStore::open(&config.workspace_label) {
        ...
        Err(e) => {
            if config.require_secure_store {
                eprintln!(
                    "[discord] secure keyring unavailable AND TURM_DISCORD_REQUIRE_SECURE_STORE=1: {e}"
                );
                ...
                Err(e) => {
                    ...
                    eprintln!(
                        "[discord] keyring credential is corrupted (failed to deserialize): {e}. \
                         Re-run `turm-plugin-discord auth` to overwrite."
                    );
                    None
                }
            },
