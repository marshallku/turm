...
//!
//! `<docs_root>` is `<TURM_TODO_ROOT>/..` — sibling-of-todos. For the
        ...
        for path in &todo.linked_kb {
            match resolve_within(root, path) {
                Some(abs) => {
                    ...
                    // posture (`turm-plugin-kb::kb::check_no_symlink_ancestors`):
...
/// location can't sneak in. Same posture as `turm-plugin-kb`'s
...
mod tests {
    ...
    fn assemble_layers_global_workspace_and_kb() {
        ...
        fs::write(docs.join("claude/workspaces/turm.md"), "WORKSPACE_RULES\n").unwrap();
